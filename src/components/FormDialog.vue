...
const props = defineProps<{
  title: string
  submitLabel?: string
  // While true, the submit button is disabled and shows "Saving...";
  // Escape / backdrop-click / Cancel are also blocked (mirrors
  // ConfirmAction) so a dialog can't be dismissed mid-mutation —
  // pre-fix an operator could close the dialog while the request was
  // in flight, lose the error surface, and re-submit a duplicate write.
  loading?: boolean
  error?: string
  wide?: boolean
    ...
    <div ref="dialogRef" :class="wide ? 'max-w-xl' : 'max-w-lg'" class="bg-white dark:bg-gray-900 dark:border dark:border-gray-700 rounded-lg shadow-lg p-6 mx-4 w-full" role="dialog" aria-modal="true" :aria-label="title" :aria-busy="loading || undefined">
      ...
      <form @submit.prevent="$emit('submit')">
        ...
        <div class="flex flex-wrap justify-end gap-2 mt-5 pt-4 border-t border-gray-200 dark:border-gray-700">
          <button type="button" @click="$emit('cancel')" :disabled="loading" class="px-3 py-1.5 text-sm text-gray-600 dark:text-gray-300 hover:text-gray-800 dark:hover:text-gray-100 rounded hover:bg-gray-100 dark:hover:bg-gray-800 cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed">Cancel</button>
          <button
            type="submit"
