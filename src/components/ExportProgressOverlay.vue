...
function onDocumentKeydown(e: KeyboardEvent) {
  if (e.key === 'Escape' && props.open && props.cancellable) emit('cancel')
}
...
<template>
  <div v-if="open" class="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
    ...
    >
      <h3 id="export-progress-title" class="text-sm font-semibold text-gray-900 dark:text-gray-100 mb-2">Assembling export…</h3>
      <p class="text-sm text-gray-600 dark:text-gray-300 mb-1" role="status" aria-live="polite">
      ...
      <div v-if="cancellable" class="mt-4 flex justify-end">
        <button
          type="button"
          class="text-xs text-gray-700 dark:text-gray-200 px-3 py-1.5 rounded border border-gray-300 dark:border-gray-600 hover:bg-gray-50 dark:hover:bg-gray-800 cursor-pointer"
          @click="$emit('cancel')"
