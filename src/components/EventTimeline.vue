...
<template>
  <div v-if="events.length === 0" class="text-sm text-gray-400 py-6 text-center">No events</div>
  <div v-else class="divide-y divide-gray-100">
    <div v-for="e in events" :key="e.event_id">
      ...
      >
        <svg class="w-3 h-3 text-gray-400 shrink-0 transition-transform" :class="expanded === e.event_id ? 'rotate-90' : ''" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
        ...
        <span class="text-gray-400 text-xs shrink-0 whitespace-nowrap" :title="new Date(e.timestamp).toISOString()">{{ formatDateTime(e.timestamp) }}</span>
      </div>
      <div v-if="expanded === e.event_id" class="pl-6 pb-2 text-xs">
        <div class="grid grid-cols-2 gap-x-6 gap-y-1 mb-2">
          ...
          <div v-if="e.scope"><span class="text-gray-400">Scope:</span> <span class="font-mono">{{ e.scope }}</span></div>
          <div v-if="e.tenant_id"><span class="text-gray-400">Tenant:</span> <TenantLink :tenant-id="e.tenant_id" /></div>
          <div v-if="e.request_id"><span class="text-gray-400">Request ID:</span> <span class="font-mono">{{ e.request_id }}</span></div>
          <div v-if="e.correlation_id"><span class="text-gray-400">Correlation ID:</span> <span class="font-mono">{{ e.correlation_id }}</span></div>
