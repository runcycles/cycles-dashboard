  ...
  <div>
    ...
    <form @submit.prevent="query" class="card p-4 mb-4">
      ...
      <div class="flex gap-2 mt-3 pt-3 border-t border-gray-100">
        <span class="text-xs text-gray-600 dark:text-gray-400 py-1">Quick range:</span>
    ...
    <div v-if="!loading" class="flex items-center justify-between mb-2">
      <p class="text-xs text-gray-600 dark:text-gray-400">{{ entries.length }} result{{ entries.length !== 1 ? 's' : '' }}</p>
    ...
    <div class="card-table">
      <table class="w-full text-sm min-w-[900px]">
        ...
        <tbody class="divide-y divide-gray-100">
            ...
            <tr v-if="expanded === e.log_id" class="bg-gray-50/70">
              <td :colspan="7" class="table-cell pl-11">
                <div class="grid grid-cols-2 gap-x-6 gap-y-1 text-xs mb-3">
                  ...
                  <div v-if="e.user_agent"><span class="text-gray-600 dark:text-gray-400">User Agent:</span> {{ e.user_agent }}</div>
                  <div v-if="e.error_code"><span class="text-gray-600 dark:text-gray-400">Error Code:</span> <span class="font-mono text-red-500">{{ e.error_code }}</span></div>
                  <div v-if="e.resource_type"><span class="text-gray-600 dark:text-gray-400">Resource Type:</span> {{ e.resource_type }}</div>
                  <div v-if="e.resource_id"><span class="text-gray-600 dark:text-gray-400">Resource ID:</span> <span class="font-mono">{{ e.resource_id }}</span></div>
                </div>
                <div v-if="e.metadata && Object.keys(e.metadata).length > 0" class="bg-white border border-gray-200 rounded p-3 text-xs font-mono overflow-auto max-h-48">
                  <div class="text-gray-600 dark:text-gray-400 mb-1 font-sans text-xs">Metadata</div>
          ...
          <tr v-if="loading">
            <td colspan="7" class="px-4 py-12 text-center text-gray-600 dark:text-gray-400">Loading...</td>
          </tr>
        </tbody>
      </table>
    </div>
    ...
    <div v-if="showExportConfirm" class="fixed inset-0 bg-black/40 flex items-center justify-center z-50" @click.self="showExportConfirm = null">
      <div class="bg-white dark:bg-gray-900 dark:border dark:border-gray-700 rounded-lg shadow-lg p-6 max-w-sm mx-4">
        ...
        <p class="text-xs text-gray-600 dark:text-gray-400 mb-4">Exported files contain unmasked sensitive data. Handle with care.</p>
