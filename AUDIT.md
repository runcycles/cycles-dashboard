...
**What comes next.** This closes every scale-hardening polish item that can ship without a server or spec change. Remaining work (task #20) is the server-spec blocker set: W1 (bulk filter-in-body for select-all-matching), W3 (global "find tenant" `search=` param), R7-SSE stretch.
