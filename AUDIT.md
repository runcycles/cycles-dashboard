...
**Date:** 2026-04-18 (v0.1.25.37 release — per-row "Copy JSON" button on every triage surface that renders event / audit / delivery rows. Operator feedback: *"one thing we need for triage is ability to copy individual event, audit logs as json into clipboard, we do have export but its many entries, often you just want to copy all data from an individual event/log."* Follow-up: *"Event Timeline in Budget Detail also needs this function Copy JSON, check other event, audit components where its needed, Delivery History, etc, where triag maybe needed."* Landed on four surfaces: **(1) EventsView** (list), **(2) AuditView** (list), **(3) EventTimeline** (component used in BudgetDetail), **(4) WebhookDetailView delivery history** (flat rows, inline trailing-column button). EventsView previously had a `Copy` button inside the `Data` sub-box that copied only `event.data`; widened to a row-level `Copy JSON` button in the expanded-panel header that copies the **full Event object** via `safeJsonStringify(event)` (event_id, event_type, category, scope, tenant_id, source, timestamp, request_id, correlation_id, actor, data). The data-only copy is subsumed — keeping both would have been visual noise. AuditView had no copy button at all; new matching `Copy JSON` in the expanded-panel header copies the **full AuditLogEntry including `metadata`** — critical because bulk-action rows stow per-row outcomes in `metadata.succeeded[] / .failed[] / .skipped[]` which is where most triage data lives and isn't reachable via any search filter (audit `search` scans `resource_id` / `log_id` / `operation` / `error_code` only). Both buttons reuse the same pattern: `copiedEventId` / `copiedLogId` ref flips to the row's id on click, label swaps `Copy JSON` → `Copied!` for 2s via `setTimeout`, clipboard errors silently swallowed (permission-denied / insecure-context fallback is select-and-copy from the existing `<pre>` blocks). **Tests.** New `src/__tests__/copy-row-as-json.test.ts` — 3 specs: EventsView Copy JSON writes the full event object (asserts `event_id`, `event_type`, `tenant_id`, `actor`, `data` all present in the serialized payload) + label toggle; AuditView Copy JSON writes the full entry with metadata preserved (asserts `log_id`, `operation`, `tenant_id`, `metadata.succeeded`, `metadata.failed[0].error_code` all present) + label toggle; AuditView silently swallows `writeText` rejections (permission-denied path doesn't throw). **No spec/server dependency.** Compose admin `.29` + server `.13` pins from v0.1.25.36 hold. Compose admin lift to `0.1.25.30` paired into this PR since admin-server `.30` landed earlier the same day and the dashboard stack was already running against it locally. **Validation gates.** vitest **688 / 688 passing** across 54 files (+3 from v0.1.25.36's 685/53); typecheck clean; build clean. **Version bumps.** `package.json` `0.1.25.36 → 0.1.25.37`; `package-lock.json` synced; `docker-compose.prod.yml` dashboard self-image `.36 → .37` + admin `.29 → .30`; `docker-compose.yml` admin `.29 → .30`; `README.md` compose-example dashboard image `.36 → .37` + cycles-admin `.28 → .30` (catches a stale README drift that had never been bumped across v0.1.25.33–.36).), 2026-04-18 (v0.1.25.36 release — BudgetsView row-select + bulk Freeze / Unfreeze, closing the pattern-gap that v0.1.25.35 shipped only the filter-apply path while TenantsView + WebhooksView already had both row-select and filter-apply. Three spec-defined budget bulk actions (`CREDIT` / `DEBIT` / `RESET` / `RESET_SPENT` / `REPAY_DEBT`) live on the filter-apply path from v0.1.25.35; Freeze + Unfreeze are deliberately NOT in the server-side `BUDGET_BULK_ACTIONS` enum per spec v0.1.25.26 (they're status transitions, not balance mutations), so the dashboard fans them out client-side via `rateLimitedBatch` over the existing per-row `freezeBudget` / `unfreezeBudget` endpoints — bounded concurrency, 429-aware, with per-row eligibility gating (`status === 'ACTIVE'` for Freeze, `status === 'FROZEN'` for Unfreeze, `CLOSED` rows skipped). **BudgetsView row-select state** (`src/views/BudgetsView.vue`). `selected: Set<string>` ledger-id set with O(1) toggle; filter-change watchers (`selectedTenant`, `filterStatus`, `filterUnit`, `filterScope`, `search`, `filterUtilMin`, `filterUtilMax`, plus route-query `?filter=`) clear the selection so stale ids never leak across filter changes; watched on raw refs (not debounced) so a typing operator sees selection clear immediately. New 40px checkbox column added as the first column of the `canManage` grid-template (`40px minmax(140px,1fr) ...`); `aria-colcount` bumped 8 → 9 when manage-cap is active. Header-row "select all visible" checkbox toggles the union of all currently-visible ledger_ids; per-row checkboxes carry `aria-label="Select budget ${scope}"` for screen-reader parity. Follows the virtualized-table pattern already proven in TenantsView.vue:147–210 so row heights, focus order, and `aria-rowcount` stay intact. **Floating bulk toolbar** (`src/views/BudgetsView.vue`, new `<Teleport to="body">` block). `<Transition>`-wrapped `role="toolbar" aria-label="Bulk budget actions"` bar pinned to the viewport via `fixed bottom-8 left-1/2 -translate-x-1/2`, visible whenever `canManage && selectedVisibleCount > 0`. Shows `{{ selectedVisibleCount }} selected` counter + Freeze button (red-700 danger) + Unfreeze button (green-700) + Clear-selection X button. Teleported so the toolbar doesn't move with the virtualized scroll container. **Bulk status flow** (`src/views/BudgetsView.vue`, new state + handlers). `bulkStatusAction: 'freeze' | 'unfreeze' | null`, `bulkStatusProgress: { done, total, failed }`, `bulkStatusRunning: boolean`, private `bulkStatusAbort: AbortController` for mid-run cancellation. `openBulkStatus(action)` opens a ConfirmAction modal with dynamic title "Freeze N budgets?" / "Unfreeze N budgets?" using `bulkStatusTargets().length` (ACTIVE-filtered for Freeze, FROZEN-filtered for Unfreeze — CLOSED is terminal and always skipped). `executeBulkStatus()` fans out via `rateLimitedBatch(targets, fn, { concurrency: 4 })` over `freezeBudget(tenant, ledgerId, 'bulk')` / `unfreezeBudget(tenant, ledgerId, 'bulk')` (reason string `'bulk'` preserves audit-log context); progress updates after every resolved promise; post-run toast summary ("Froze 7 of 9 budgets — 2 failed, see console for details" when any failed, "Froze 9 budgets" on full success); per-failure `console.warn` with `ledgerId + scope + error`; `selected.value = new Set()` + `loadList()` on completion. `cancelBulkStatus()` calls `bulkStatusAbort.abort()` mid-run or resets state when idle. Matches TenantsView's established row-select reporting pattern (toast + console.warn for triage) rather than the filter-apply path's `BulkActionResultDialog` — `BulkActionResultDialog` is response-driven and requires the server's split succeeded/failed/skipped arrays, which don't exist in the client-side fan-out model. **Tests.** New `src/__tests__/BudgetsView-row-select-bulk.test.ts` — 10 specs covering: (1) toolbar hidden without selection, (2) toolbar visible with Freeze + Unfreeze + counter + Clear on selection, (3) select-all header toggle unions all visible ledger_ids, (4) clicking Clear X empties selection, (5) filter-change (tenant, status, unit, scope, search) clears selection, (6) bulk Freeze fans out only for ACTIVE rows (FROZEN + CLOSED rows in the selection skipped), (7) bulk Unfreeze fans out only for FROZEN rows (ACTIVE + CLOSED skipped), (8) reason string `'bulk'` passed through to per-row freeze/unfreeze call, (9) selection cleared + `loadList()` reloaded after success, (10) per-failure `console.warn` fires + toast summary mentions partial counts. Test helpers: `toggleRow(w, scope)` uses `aria-label="Select budget ${scope}"`; `selectTenant(w, id)` via `select#budget-tenant`; `Teleport: true` stub so tests can query `[role="toolbar"]` in the default slot. **Validation gates.** vitest **674 / 674 passing** across 53 files (up from 664 / 52 in v0.1.25.35; +10 new specs in this release); typecheck clean; build 926ms; BudgetsView chunk grew 40.74 → 45.33 kB (+4.59 kB, gzip 13.32 kB). **Compose pins unchanged.** Admin `.29` + server `.13` stays the shipped baseline from v0.1.25.35 — no new spec features exercised, the row-select Freeze/Unfreeze path uses per-row endpoints that have been stable since v0.1.25.19. **Spec alignment unchanged at v0.1.25.26** — the Freeze/Unfreeze UX is a dashboard-side shape decision on top of existing per-row endpoints; spec badge moves only when new server-side endpoints or wire shapes land. **Version bumps.** `package.json` `0.1.25.35 → 0.1.25.36`; `package-lock.json` synced via `npm install --package-lock-only`; `docker-compose.prod.yml` dashboard self-image `0.1.25.35 → 0.1.25.36` (admin + cycles-server pins unchanged); `docker-compose.yml` unchanged (dev stack uses local `build:`); `README.md` compose-example dashboard image `0.1.25.35 → 0.1.25.36`.), 2026-04-18 (v0.1.25.35 release — Budget bulk-action UI (Slice C of the cycles-governance-admin v0.1.25.23–.26 integration plan). Ships the dashboard counterpart to cycles-governance-admin's v0.1.25.26 `POST /v1/admin/budgets/bulk-action` endpoint (admin-server `.29` shipped 2026-04-18). Five server-side actions (`CREDIT`, `DEBIT`, `RESET`, `RESET_SPENT`, `REPAY_DEBT`) exposed as a filter-apply path alongside the existing row-select fund flow; `BudgetBulkFilter.tenant_id` is REQUIRED per spec (no cross-tenant fan-out), so the "Bulk action…" toolbar button is disabled whenever `selectedTenant.value === ''` with tooltip "Select a tenant to bulk-act on budgets" — preserves the cross-tenant `over_limit` / `has_debt` list view intact (operators scan the incident, then drill into a tenant to act). **Types** (`src/types.ts` +61 lines). New `BUDGET_BULK_ACTIONS` readonly tuple + `BudgetBulkAction` type, `BudgetBulkFilter` interface with REQUIRED `tenant_id`, `BudgetBulkActionRequest` (`amount` for CREDIT/DEBIT/RESET/REPAY_DEBT, `spent` for RESET_SPENT only, optional `reason`, `expected_count` preflight, `idempotency_key`), `BudgetBulkActionResponse` reusing existing `BulkActionRowOutcome` for succeeded/failed/skipped arrays. **API wrapper** (`src/api/client.ts` — new `bulkActionBudgets(body)` mirroring the tenants + webhooks wrappers). **BudgetsView flow** (`src/views/BudgetsView.vue` +374 lines). Two-step dialog: `FormDialog` setup step collects action + amount (hidden for RESET_SPENT) + spent (RESET_SPENT only) + reason; on submit, transitions to shared `BulkActionPreviewDialog` (reused from Slice A / v0.1.25.28) which walks `listBudgets({ tenant_id, …filter })` pages, applies per-action status eligibility (`ACTIVE` only for CREDIT/DEBIT/REPAY_DEBT; all statuses for RESET/RESET_SPENT), shows filter summary, live count, first-10 sample, expected_count arm. On confirm, POSTs `BudgetBulkActionRequest` with filter including REQUIRED `tenant_id`, action-conditional `amount` / `spent`, optional `reason`, `idempotency_key` (UUID v4 via `generateIdempotencyKey`), and `expected_count` only when the walk reached end (no maxPages truncation). **Error paths.** Request-level `LIMIT_EXCEEDED` + `COUNT_MISMATCH` route through Slice-B's `formatBulkRequestError('budgets', 500, details)` for humanized prose; any non-empty `failed[]` / `skipped[]` arrays open Slice-B's `BulkActionResultDialog` with per-row `error_code` prose from `formatErrorCode` — `BUDGET_EXCEEDED` (DEBIT over remaining balance), `INVALID_TRANSITION` (non-ACTIVE row under CREDIT/DEBIT/REPAY_DEBT despite the client-side eligibility gate — server is authoritative), `INTERNAL_ERROR` fallback. Confirm-danger styling on the preview dialog for DEBIT action (irreversible balance mutation). **Slice D verification (sentinel split).** No-op — `TenantLink.isSystem` guard (`src/components/TenantLink.vue:13–15`) checks `id.startsWith('__')` which catches both `__admin__` and `__unauth__` sentinels introduced in spec v0.1.25.25; grep confirmed no other code paths reference the literals. OverviewView Recent-Activity and Recent-Denials render `tenant_id` via `TenantLink`. AuditView exposes no tenant_id datalist (free-text input only). No regression, no code change — documentation-only verification bundled into this release. **Tests.** New `src/__tests__/BudgetsView-bulk-action.test.ts` — 9 specs covering: (1) Bulk-action button disabled without tenant selection (spec tenant_id REQUIRED), (2) enables once a tenant is selected, (3) happy-path DEBIT submits filter + amount + idempotency_key + expected_count, (4) RESET_SPENT shows `spent` input / hides `amount` input / omits `amount` from request body, (5) LIMIT_EXCEEDED humanized via `formatBulkRequestError`, (6) COUNT_MISMATCH humanized via `formatBulkRequestError`, (7) BulkActionResultDialog opens with per-row `BUDGET_EXCEEDED` prose when server returns failed rows, (8) idempotency_key regenerated between submits (prevents replay-window collisions across operator retries), (9) omits `expected_count` from body when walk hits maxPages (only sends preflight gate when the sample is complete). `src/__tests__/client.test.ts` +4 specs for `bulkActionBudgets` wire-format: POST to `/v1/admin/budgets/bulk-action` with filter + action + amount + idempotency_key; RESET_SPENT variant omits `amount` and includes `spent`; `LIMIT_EXCEEDED` error surfaces `errorCode` + `details.total_matched` on the thrown `ApiError`; `COUNT_MISMATCH` error surfaces `errorCode`. **Compose bumps.** `docker-compose.prod.yml` + `docker-compose.yml` admin pin `0.1.25.28 → 0.1.25.29` so both compose variants ship the required admin baseline; cycles-server pin `.13` unchanged. **Spec alignment bump.** Badge `v0.1.25.25 → v0.1.25.26` — the dashboard now exercises the v0.1.25.26 `POST /v1/admin/budgets/bulk-action` endpoint end-to-end. **Validation gates.** vitest **664 / 664 passing** across 52 files (+13 new from v0.1.25.34's 651/51: 9 in `BudgetsView-bulk-action.test.ts`, 4 in `client.test.ts`); typecheck clean, build successful. **Version bumps.** `package.json` `0.1.25.34 → 0.1.25.35`; `package-lock.json` synced; `docker-compose.prod.yml` dashboard `.34 → .35` + admin `.28 → .29`; `docker-compose.yml` admin `.28 → .29`; `README.md` spec badge + description `v0.1.25.25 → v0.1.25.26` + new "Bulk budget action (CREDIT / DEBIT / RESET / RESET_SPENT / REPAY_DEBT)" row in the Operational Actions table citing admin `.29+` requirement.), 2026-04-18 (v0.1.25.34 release — Per-row bulk outcome rendering (Slice B of the cycles-governance-admin v0.1.25.23–.26 integration plan). Replaces the `console.warn`-only per-row failure reporting on the tenants + webhooks filter-apply bulk paths with a proper triage dialog. **New shared component** `src/components/BulkActionResultDialog.vue` — opens after a bulk submit returns any non-empty `failed[]` or `skipped[]` array. Succeeded count renders as a green summary badge (no enumeration — the refreshed list view already shows succeeded rows in their new state); failed rows render inside a `<details>` block (defaults open when any failures exist) with per-row id, Copy-ID button (writes to `navigator.clipboard` with 2s "Copied" flash), and canonical prose mapped from `error_code` via new shared util; skipped rows render inside a second `<details>` block (defaults open iff no failures exist) with the server-supplied `reason` per row. Uses the same `useFocusTrap(dialogRef)` pattern as `BulkActionPreviewDialog` + `ConfirmAction`, emits `close` on overlay click, Close button, or Escape. **New shared util** `src/utils/errorCodeMessages.ts` — single source of truth for operator-facing prose on the bulk error-code catalogue. `formatErrorCode(code, message)` covers per-row codes `BUDGET_EXCEEDED` (v0.1.25.23 enum addition, pre-wired for Slice C's budget bulk-action where per-row failures are the rule not the exception), `INVALID_TRANSITION` (v0.1.25.22), `INTERNAL_ERROR` (v0.1.25.21), plus forward-compat fallback that renders unknown codes as `code: message` so operators still get enough detail to triage newly-introduced server codes without a dashboard bump. `formatBulkRequestError(code, itemNounPlural, serverMaxPerRequest, details)` covers request-level safety gates `LIMIT_EXCEEDED` (embeds `total_matched` when server echoes it) and `COUNT_MISMATCH` (capitalized-plural drift explainer) — migrated from the inline branches in TenantsView + WebhooksView so both views route through one helper. **Retrofits.** `TenantsView.vue` + `WebhooksView.vue` `executeFilterBulk` handlers now: (1) surface the toast summary without the "check console for details" suffix (replaced with "see details"), (2) drop the `for (const f of res.failed) console.warn(...)` enumeration, (3) open `BulkActionResultDialog` whenever `res.failed.length || res.skipped.length` is non-empty, (4) route the catch block's `LIMIT_EXCEEDED`/`COUNT_MISMATCH` humanization through `formatBulkRequestError`. Preview dialog closes first; result dialog renders as a separate overlay with its own focus trap. **Tests.** New `src/__tests__/errorCodeMessages.test.ts` — 18 specs covering the catalogue + forward-compat `code: message` fallback for unknown codes + safety assertions that the formatter never throws on arbitrary code strings. New `src/__tests__/BulkActionResultDialog.test.ts` — 11 specs covering succeeded summary, failed-row prose (including unknown `FUTURE_CODE` forward-compat), skipped-row reason, failed-opens-by-default when both sections populated, skipped-opens-by-default when no failures, Copy-ID clipboard write, close on button/Escape/overlay click, total_matched in header when rows are truncated. **Validation gates.** vitest **651 / 651 passing** across 51 files (+29 new from v0.1.25.33's 622/49); typecheck clean, build 916ms. **Governance-spec alignment unchanged at v0.1.25.25** — Slice B is dashboard-UX groundwork for Slice C; spec badge bumps when Slice C lands (pairs with admin-server v0.1.25.29). **Compose pins unchanged** — admin `.28` + server `.13` stays the shipped baseline. Image published as `ghcr.io/runcycles/cycles-dashboard:0.1.25.34` via the existing `release.yml` tag-push workflow. **Version bumps.** `package.json` `0.1.25.33 → 0.1.25.34`; `package-lock.json` synced via `npm install --package-lock-only`; `docker-compose.prod.yml` dashboard self-image `0.1.25.33 → 0.1.25.34` (admin/server pins unchanged); `README.md` compose-example dashboard image `0.1.25.33 → 0.1.25.34`.), 2026-04-18 (v0.1.25.33 release — AuditView filter DSL completeness against cycles-governance-admin v0.1.25.24 (admin-server .27+). Closes the three remaining dashboard-side gaps on the audit-log filter DSL: **(1) `error_code_exclude` NOT-IN-list** — new input field in row 2 of the AuditView filter toolbar, shares the `audit-error-code-options` datalist with `error_code` so typeahead covers both inputs, spec-per semantics mean NULL entry `error_code` (success rows) always passes the exclude predicate so hiding noisy codes never silently hides successes; **(2) `operation` array promotion** — input field flipped from scalar to comma-separated IN-list (explode=false wire format, maxItems 25 server-enforced, single-token input stays byte-compatible with the pre-.24 scalar shape so deep-links from OverviewView or hand-crafted URLs continue to work); **(3) `resource_type` array promotion** — control flipped from `<select>` (hard-coded 6 options) to `<input list=...>` with `<datalist>` typeahead over the same 6 known resource types, deliberately non-whitelist so servers adding new resource types don't regress the control; shared `normalizeList(raw: string)` helper de-dupes the four array filters' comma/whitespace splits into one trim/split/filter/dedupe pipeline. Row 2 template restructured — the old `sm:col-span-2` Status+Submit cell dropped to free col 4 for the new Error Code (Exclude) input; Status band chip group + Run Query button moved to a new flex-wrap row 3 so the status chips don't collide with row 2 inputs on narrow viewports. `applyQueryParams` accepts `?error_code_exclude=` from URL so OverviewView Recent-Denials pill patterns can layer NOT-IN-lists on deep-links (pattern reserved — no OverviewView caller yet, but the wire is in place for the follow-up). vitest **622 / 622 passing** across 49 files (+16 new: 5 error_code_exclude specs, 3 operation-array specs, 4 resource_type-array specs, 1 URL hydration spec for error_code_exclude, 3 URL deep-link smoke specs added to `AuditView-url-deeplink.test.ts`); typecheck clean, build 878ms. Governance-spec alignment unchanged at v0.1.25.25 — the admin filter DSL landed in spec v0.1.25.24 but the dashboard's spec-badge pin is set by the most recent *end-to-end* supported spec, not the most recent feature-landing spec; Slice B (per-row bulk outcome rendering) + Slice C (budget bulk-action, spec v0.1.25.26) will bump the badge. Admin-server baseline unchanged at .28 (shipped v0.1.25.32 compose); .27+ is the DSL-support floor, .28 is the audit-sentinel-split ship. Image tag `ghcr.io/runcycles/cycles-dashboard:0.1.25.33` publishes via existing `release.yml` on tag push. **Version bumps.** `package.json` `0.1.25.32 → 0.1.25.33`; `package-lock.json` synced via `npm install --package-lock-only`; `docker-compose.prod.yml` dashboard self-image `0.1.25.32 → 0.1.25.33` (compose admin/server pins unchanged — admin .28 + server .13 stays the shipped baseline from v0.1.25.32); `README.md` compose-example dashboard image `0.1.25.32 → 0.1.25.33`.), 2026-04-17 (v0.1.25.32 release — compose + baseline rollup, no dashboard code change. Prod compose lifts to cycles-admin `0.1.25.27 → 0.1.25.28` (audit `tenant_id` sentinel split: `<unauthenticated>` → `__unauth__` for pre-auth failures + new `__admin__` for admin-key-authenticated requests not scoped to a single tenant; legacy sentinel still queryable, double-underscore delimiters are URL-safe, tenant grammar `^[a-z0-9-]+$` excludes underscores so no real-tenant collision — dashboard's `TenantLink.isSystem` guard shipped in v0.1.25.31 already treats both the legacy angle-bracket sentinel and the new `__`-prefixed sentinels as non-drillable, so the bump is drop-in with zero client change), cycles-server `0.1.25.12 → 0.1.25.13` (`SORTED_HYDRATE_CAP = 2000` on `listReservationsSorted` — labeled break exits the SCAN loop once the capped slice fills, WARN logged naming the tenant + sort tuple, legacy no-sort-params path intentionally uncapped; plus Jackson `@JsonValue getWire()` + `@JsonCreator fromWire(String)` on `ReservationSortBy` + `SortDirection` to match the admin-plane `SortSpec`/`SortDirection` contract, wire form stays lowercase, parsing stays case-insensitive — no wire change, no dashboard change), and dashboard self-image `0.1.25.31 → 0.1.25.32`. Dev `docker-compose.yml` already landed admin `.28` + server `.13` earlier the same day via PR #86 (merged 01:11 UTC); this tag promotes the same baseline to prod. Governance-spec alignment bumps v0.1.25.24 → v0.1.25.25 (admin `.28` bumped the spec to document the sentinel split in the `tenant_id` query-param description — no new endpoints, no new wire shapes, purely a spec-text update). Zero `src/**` code change — vitest stays at **606 / 606 passing** across 49 files (includes the 41 URL deep-link smoke specs that landed earlier the same day via PR #85, also with no version bump), typecheck clean, build clean. Image tag `ghcr.io/runcycles/cycles-dashboard:0.1.25.32` publishes via the existing `release.yml` tag-push workflow. **Version bumps.** `package.json` `0.1.25.31 → 0.1.25.32`; `package-lock.json` synced via `npm install --package-lock-only`; `docker-compose.prod.yml` dashboard `0.1.25.31 → 0.1.25.32` + cycles-admin `0.1.25.27 → 0.1.25.28` + cycles-server `0.1.25.12 → 0.1.25.13`; `README.md` compose-example dashboard image `0.1.25.31 → 0.1.25.32` + cycles-admin `0.1.25.27 → 0.1.25.28` + spec badge `v0.1.25.23 → v0.1.25.25` (also catches a lingering README drift where the badge had never been bumped for the v0.1.25.24 alignment that shipped with v0.1.25.30).), 2026-04-17 (v0.1.25.31 release — Overview polish + TenantLink sentinel handling on the same landing page that v0.1.25.30 rebuilt. Five tightly-coupled post-.30 commits that together close the consistency gaps surfaced in operator review of the new "What needs attention" page: (1) **Budgets-at-Cap widened** from spec's narrow `over_limit` (debt > overdraft_limit) to `utilization_min=0.9` so exhausted-without-debt AND approaching-cap (90–99%) budgets surface on the landing page before they blow; card-level severity adapts to "at or near cap" vs "near cap" based on worst row; row metric encodes per-budget severity (red ≥100%, amber 90–99%); cap display at 5 rows with `py-1.5` density; (2) **Three budget cards grouped on row 1** — Budgets at or near cap / Budgets with debt / Frozen budgets form the top row of the 6-card grid since "state of budgets" is the single most-asked ops question at 2am; non-budget signals (Failing webhooks / Expiring API keys / Recent denials) form row 2; (3) **All three budget cards list top-5 scopes inline** — Frozen Budgets was a center hyperlink ("View N frozen budgets") with no scope names; rewrote to match the at-cap + with-debt inline-list pattern, with graceful-degradation fallback when the new `listBudgets?status=FROZEN` fetch fails but `overview.budget_counts.frozen > 0`; Budgets with Debt sliced to 5 via `debtScopesSorted` for parity; (4) **Non-budget cards sliced to 5 + row padding normalized** — Failing Webhooks and Recent Denials were rendering the full server list at `py-2` while budget cards + Expiring Keys were at 5 rows / `py-1.5`; added `failingWebhooksSorted` + `recentDenialsSorted` computeds, brought all non-budget cards to `py-1.5`; **counter-strip tile titles bumped** from `text-xs muted` to `text-sm font-medium text-gray-700` to match alert-card `<h2>` typography; (5) **Typographic + micro-color consistency sweep** — all 7 card `<h2>` titles flipped to sentence case for internal consistency + 1:1 match with banner pill labels ("Budgets At or Near Cap" → "Budgets at or near cap", "Budgets with Debt" → "Budgets with debt", etc., with "API" preserved as acronym in "Expiring API keys"); banner pill "Keys expiring" → "Expiring keys" to match the adj+noun shape of sibling axes; `tabular-nums` added to Failing Webhooks failure count + Expiring Keys day count (every right-column numeric metric is now monospaced-digit); Expiring Keys 3–7d range `text-yellow-*` → `text-amber-*` aligning with the card's border/icon/badge/banner pill (red `<=2d` urgency signal preserved); Recent Operator Activity rows `py-2` → `py-1.5` to match Recent Denials (same two-line shape). **TenantLink sentinel handling** — `isSystem` guard extended from `__`-prefixed only (`__system__`, `__root__`) to also match angle-bracket-wrapped placeholders (`<unauthenticated>`) so audit rows where a pre-auth request 401'd before key→tenant resolution don't render a broken drill-down link to `/tenants/<unauthenticated>`; one component change flows through every `TenantLink` caller (AuditView, EventsView, BudgetsView, ApiKeysView, WebhooksView, WebhookDetailView, EventTimeline); new `src/__tests__/TenantLink.test.ts` pins both sentinel conventions (5 specs). **Admin-server baseline tightened** — `docker-compose.yml` (dev) cycles-admin pin `0.1.25.26 → 0.1.25.27` to match `docker-compose.prod.yml`; the recommended admin-server requirement v0.1.25.27+ from the v0.1.25.30 note is now the shipped baseline, since every user running the compose stack will be on .27 by default. Governance-spec alignment unchanged at v0.1.25.24; admin-server requirement unchanged at v0.1.25.27+ (now required in both compose variants, not just prod). vitest **561 / 561 passing** across 43 files (+13 new across 5 commits: +2 for non-budget cap-at-5, +6 for frozen list/cap/fallback + debt slice + FROZEN fetch check, +5 for TenantLink sentinel coverage; 3 existing specs updated for Title→sentence case); typecheck clean, build 943ms. Image tag `ghcr.io/runcycles/cycles-dashboard:0.1.25.31` publishes via `release.yml` on tag push. **Version bumps.** `package.json` `0.1.25.30 → 0.1.25.31`; `package-lock.json` synced via `npm install --package-lock-only`; `docker-compose.prod.yml` dashboard image pin `0.1.25.30 → 0.1.25.31`; `docker-compose.yml` (dev) cycles-admin pin `0.1.25.26 → 0.1.25.27`; `README.md` compose-example dashboard image `0.1.25.30 → 0.1.25.31`.), 2026-04-17 (v0.1.25.30 release — Overview rebuilt as "What needs attention" (I1 + I3) landing page + Audit filter DSL wire-up (v0.1.25.24). Governance-spec alignment v0.1.25.23 → v0.1.25.24; admin-server recommended requirement v0.1.25.26+ → v0.1.25.27+ for the new audit filter dimensions (pre-.27 servers ignore unknown params per additive-parameter guarantee). AuditView gains an Error Code input with datalist typeahead over the 29-value `ErrorCode` enum (comma/whitespace-separated IN-list, `explode=false` wire format after trim/split/dedupe normalization) and a Status band select (five presets — All / 2xx / 4xx+5xx / 4xx / 5xx) that translates to `status_min`/`status_max` (dashboard sidesteps the spec's status-exact ↔ range mutex by construction — never sends exact `status`). Search field placeholder + aria-label updated to advertise the widened v0.1.25.24 match set (`resource_id, log_id, error_code, operation`). `applyQueryParams` consumes `?error_code=` + `?status_band=` from URL so deep-links land pre-filtered. New `ERROR_CODES` const in `src/types.ts` mirrors the spec enum for typeahead. **Unblocks the deferred drill-down on OverviewView** — each `recent_denials_by_reason` pill now renders as a `<router-link>` to `/audit?error_code=CODE&status_band=errors` (was an inert `<span>` in the original `recent_denials_by_reason` land-in, which deferred this on the premise that listAuditLogs couldn't filter by error_code — v0.1.25.24 landed that filter and cycles-server-admin v0.1.25.27 wired the server side). Overview rebuild details: alert-first headline banner composed over 6 axes (failing webhooks / over-limit budgets / debt-laden budgets / frozen budgets / recent denials / expiring keys) renders either "**N area(s) need attention**" or "**All clear**" so the 2am on-call answer lives above the fold; 6 alert cards with count badges + deep-link router-links — new **Expiring API Keys (7d)** card (`src/utils/expiringKeys.ts`, pure-fn + 10 unit tests, red `≤2d` / yellow `3–7d`), new **Recent Operator Activity** card (closes I3 — `listAuditLogs({ limit: 10 })`, raw `font-mono` operation format matching `AuditView.vue:409` so copy-paste→filter works, deep-links into tenant-detail / webhook-detail / `/audit?search=<log_id>`); counter tiles preserved as a compact 4-up strip under the status banner — **symmetrical per-tile chip breakdown** (Tenants ACTIVE/SUSPENDED/CLOSED, Budgets ACTIVE/FROZEN/DEBT, Webhooks ACTIVE/PAUSED/DISABLED, Events by category) wired via new `?status=` URL params on TenantsView / WebhooksView and existing `?status=` + `?filter=` + `?category=` params elsewhere, zero-count chips omitted so each tile reads as a one-glance state breakdown, WebhooksView also gains `?failing=1` + "Failing only" checkbox so failing-webhooks drill-down lands on a pre-filtered list, chip color taxonomy centralized in `src/style.css` (`.chip-success` / `.chip-warning` / `.chip-danger` / `.chip-neutral` / `.chip-category`); graceful degradation via `Promise.allSettled` so a flaky `/api-keys` or `/audit` endpoint surfaces an inline error banner without blanking the landing page; new `recent_denials_by_reason` pill row on the Recent Denials card (server-populated since cycles-governance-admin v0.1.25.8, dashboard was silently dropping it — `AdminOverviewResponse.recent_denials_by_reason?: Record<string, number>` now declared in `src/types.ts`, `denialReasons` computed sorts desc, `.chip-danger` pills with `×N` tabular-nums + `title` tooltip); Recent Operator Activity operation format flipped from `humanizeOperation()`-capitalized back to raw enum in `font-mono` so it matches AuditView's column renderer verbatim (fixes cross-reference friction + enables copy-paste→Audit-filter); EventsView gains a visible **Type** filter input with `<datalist>` typeahead over the 40 `EventType` enum values (sourced from `src/types.ts:415` `EVENT_TYPES`, bound to existing `eventType` ref that already reads from `route.query.type` — closes the gap where `/events?type=reservation.denied` deep-links silently filtered with no operator-visible cue); "Recent Reservation Expiries (1h)" card dropped from Overview as low-signal (wrong-plane link, near-always-empty), grid flattened to single-column for Recent Operator Activity; ReservationsView gains defensive `?status=` URL param wiring validated against `RESERVATION_STATUSES` const so the follow-up Reservations tile slots in without another view change; full-Option-1 `reservation_counts` on `AdminOverviewResponse` deferred — blocked on a cross-repo architectural choice (cycles-server-admin has no `ReservationRepository`, reservations are per-tenant Redis state in cycles-server with no `reservations:_all` index — deferred to a spec + server PR); vitest **485 / 485 passing** across 39 files (+38 new across 3 commits in this release: 32 for the I1/I3 rebuild covering alert-banner states / Expiring Keys / Recent Activity / counter-strip DOM order / per-tile chip layout / zero-count omission / graceful degradation, +3 for `recent_denials_by_reason` populated-sorted-desc + absent-field + empty-object, +1 guard that the old expiries card stays removed, +2 for raw-enum operation format matching AuditView), typecheck clean, build 890ms; image tag `ghcr.io/runcycles/cycles-dashboard:0.1.25.30` published via `release.yml` on tag push, governance-spec alignment unchanged at v0.1.25.24, admin-server requirement unchanged at v0.1.25.26+. **Version bumps.** `package.json` `0.1.25.29 → 0.1.25.30`; `package-lock.json` synced via `npm install --package-lock-only`; `docker-compose.prod.yml` dashboard image pin `0.1.25.29 → 0.1.25.30`; `README.md` compose-example dashboard image `0.1.25.29 → 0.1.25.30`. `docker-compose.yml` (dev stack) intentionally not bumped — uses local `build:` rather than the published image.), 2026-04-17 (v0.1.25.29 release — dashboard-only UX consistency pass: new `RowActionsMenu` kebab component (Teleport-based menu, WAI-ARIA `role="menu"` + `role="menuitem"`, dark-mode parity via `src/style.css` component classes after scoped `:global(.dark)` proved unreliable through the Teleport boundary), adopted across 5 list views (ApiKeysView / ReservationsView / TenantsView / WebhooksView / BudgetsView) with status-gated items + danger-styled destructive items below a separator; BudgetsView kebab gains a `Fund` action that opens the existing fund-dialog targeting the row's budget rather than the active detail (new `fundTarget` ref decouples the dialog from `detail.value` so list-mode invocation works without entering detail mode first); detail-view header consolidation — TenantDetailView surfaces `Create Budget / Create API Key / Create Policy` in the page header action row contextual to the active tab (was middle-of-page strips), BudgetsView Fund Budget moved to detail header alongside Edit/Freeze/Unfreeze (was middle-of-card border-t strip), WebhookDetailView delivery-history filter inlined into the table-chrome row (was a 60px standalone card holding one dropdown), Export CSV/JSON button labels normalized across the dashboard (was `CSV`/`JSON` on WebhookDetailView only — now matches Audit/Budgets/Tenants/ApiKeys/Events/Reservations); AuditView filter compacted from 5-stack to a 4-column `grid-cols-1 sm:grid-cols-2 md:grid-cols-4` with a footer row holding Quick range + Run Query (`fieldset`/`legend` wrappers dropped — logical grouping preserved via field ordering, not separate containers), Log ID surfaced as the top-left field in the expand block so the existing `search` filter (`resource_id` OR `log_id` substring match per server) corresponds to a visible data field; SortHeader gains `whitespace-nowrap` on the inner span so labels like "HTTP Code" never wrap project-wide; tests — new `src/__tests__/RowActionsMenu.test.ts` covers menu open/close + a11y attrs + click dispatch, `src/__tests__/error-surfacing.test.ts` rewritten with a `clickMenuItem` helper that opens the kebab and clicks the teleported menuitem (4 ApiKeysView toast specs migrated), 4 Playwright e2e specs migrated to kebab clicks (api-keys-edit, audit-drilldown-from-apikey, budgets-freeze-unfreeze, replay-event, reservations-force-release, revoke-key); vitest 402 / 34 files (+15 from .28: +RowActionsMenu suite + reshaped error-surfacing assertions), typecheck clean, build 875ms; image tag `ghcr.io/runcycles/cycles-dashboard:0.1.25.29` published via `release.yml` on tag push, governance-spec alignment unchanged at v0.1.25.23, admin-server requirement unchanged at v0.1.25.26+), 2026-04-17 (v0.1.25.28 release — ships PR-B bulk-action wire-up + CI bulk-action smoke-probes + README/compose version realignment; governance-spec alignment bumped v0.1.25.14 → v0.1.25.23; dashboard image tag `ghcr.io/runcycles/cycles-dashboard:0.1.25.28` published via `release.yml` on tag push, with the `smoke-test-published` job verifying the tagged image through `scripts/e2e-probes.sh` — now includes 6 new probes asserting bulk-action empty-filter→400 + invalid-action→400 + zero-match→200 + idempotency replay + webhook mirror; vitest 387 / 33 files (+5 bulk-action wrapper tests), touched-layer coverage up 86.18→87.29% on client.ts, typecheck clean, build 857ms), 2026-04-17 (bulk-action wire-up PR-B undrafted — TenantsView + WebhooksView gain a server-side filter-apply path alongside the existing row-select path: single POST to `/v1/admin/tenants/bulk-action` and `/v1/admin/webhooks/bulk-action` per cycles-governance-admin v0.1.25.21 spec, filter-only body (minProperties=1), 500-row hard cap, `idempotency_key` generated via `src/utils/idempotencyKey.ts` (UUID v4 with crypto.randomUUID + getRandomValues + Math.random fallback), split succeeded/failed/skipped arrays surfaced in toast; `rateLimitedBatch` row-select path intact — filter-apply is additive; filter-apply button disabled for dashboard pseudo-values (`__root__`, `__system__`) and wildcard url-filters with no server-side equivalent; cycles-server-admin v0.1.25.26 now pinned in both compose files, PR-B undrafted and ready for merge; vitest 382 passing (up from 376 — 6 new idempotency-key tests), typecheck + build clean), 2026-04-17 (search wire-up PR-A — 6 admin list views (Tenants/Budgets/ApiKeys/Audit/Webhooks/Events) now forward the free-text `search` query param introduced in cycles-governance-admin v0.1.25.21, case-insensitive substring match on each endpoint's natural identifier pair per spec; page-1 refetch on debounced-filter change to honor cursor-tuple invalidation; pre-0.1.25.21 admin servers MUST ignore the unknown param per additive-parameter guarantee, client-side fallback filters kept for graceful degradation; vitest stays at 376 passing, build clean; PR-B bulk-action held until server .26), 2026-04-17 (tenants children breadcrumb — TenantDetailView "+N more" and inline child links now thread `?parent=<src>` to the target route so the back arrow on the landing view (filtered /tenants list or child tenant-detail) returns to the source parent instead of defaulting to flat `/tenants`. Closes the "click into Children, no way back" gap reported by an operator after W3 landed. Pure dashboard change, no spec or server dependency; vitest full suite stays at 376 passing, build clean), 2026-04-16 (non-spec scale polish — operator-facing cancel button on long multi-page exports via AbortController-observed loop, SortHeader keyboard activation (Enter/Space) pinned for the `as='div'` virtualized-grid variant, dark-mode gaps closed across EventsView + AuditView filter inputs / expanded-row borders / category badges / export-toolbar hover states; post-review fixes: AuditView status column widened 110→160px + `min-w-0 overflow-hidden` on the cell (stops `401 UNAUTHORIZED` overflow from forcing double horizontal scrollbar on narrow viewports), BudgetDetail EventTimeline `overflow-auto`→`overflow-y-auto` + `min-w-0` on flex-1 truncate (kills the phantom horizontal scrollbar), Audit + Events export CSV/JSON buttons moved from mid-page toolbar into PageHeader #actions slot so export lives next to Refresh across every list view — all dashboard-only, no server or spec change), 2026-04-16 (V4 stage 2 — six admin-plane views (Tenants, Budgets, Events, Audit, ApiKeys, Webhooks) now wired to server-side sort against cycles-server-admin v0.1.25.24 listX sort_by/sort_dir enums; TenantsView Parent + Children columns demoted to plain headers because they're client-derived and have no server-side index; compose stack bumped to cycles-server 0.1.25.12 to unblock reservations sort e2e under the runtime plane), 2026-04-16 (V4 stage 1 — useSort server-side sort opt-in with onChange callback, ReservationsView wired as reference implementation against cycles-server v0.1.25.12+ sort_by/sort_dir on GET /v1/reservations; no client-side re-sort destroying cursor tie-breaker), 2026-04-16 (scale phase 1 wire-up — ApiKeysView + BudgetsView consume the cross-tenant `/v1/admin/api-keys` and `/v1/admin/budgets` endpoints from cycles-server-admin v0.1.25.22, push all filters server-side, render `BudgetLedger.tenant_id` as a first-class column per cycles-governance-admin v0.1.25.19 / cycles-server-admin v0.1.25.23), 2026-04-16 (TenantDetailView — Edit API Key in-place, ports the v0.1.25.24 ApiKeysView diff-before-patch flow), 2026-04-16 (phase 5 polish — multi-row expansion on Events / Audit / EventTimeline for triage comparison workflow), 2026-04-16 (phase 5 polish — BudgetDetail EventTimeline virtualization + flex-fill + Load more label parity with list views), 2026-04-16 (phase 5 polish — PageHeader `itemNounPlural` override fixes "log entrys"→"log entries", WebhooksView subscription→webhook noun consistency, filter toolbars wrapped in card across TenantsView/WebhooksView/ReservationsView), 2026-04-16 (scale-hardening phase 5 — unified table-layout: flex-fill viewport on all 7 list views, fix AuditView double horizontal scrollbar, standardize Load more label), 2026-04-16 (scale-hardening phase 4 — W4 bulk-op bounded concurrency + 429 backoff across all three bulk runners, W5 reveal-timer cleanup, W6 a11y row-count live region), 2026-04-16 (scale-hardening phase 3 — V5 debounce composable, V6 PageHeader result counts, V7 filter-aware EmptyState), 2026-04-16 (scale-hardening phase 2c — V1 virtualization for EventsView + AuditView with measureElement for expandable rows), 2026-04-16 (scale-hardening phase 2b — row virtualization across 5 list views via @tanstack/vue-virtual), 2026-04-16 (scale-hardening phase 2 — pagination on tenants/webhooks/budget-detail events, lazy tabs, O(1) parent lookup, copy-event-data), 2026-04-16 (scale-hardening phase 1 — pagination, cancellation, N+1 mitigation across 6 views), 2026-04-15 (v0.1.25.27 — RESET_SPENT funding operation support, semantics corrected post-test), 2026-04-14 (error-surfacing + SecretReveal + 3 incident-response Playwright flows), 2026-04-14 (capability-gated UI visibility test layer), 2026-04-14 (v0.1.25.26 style consolidation + dark-mode restore), 2026-04-14 (a11y ratchet to WCAG AA all-levels — TERMINAL), 2026-04-14 (a11y ratchet to WCAG AA moderate+), 2026-04-14 (a11y ratchet to WCAG AA serious+critical), 2026-04-14 (v0.1.25.25 complete PERMISSIONS + unknown-filter on edit), 2026-04-14 (v0.1.25.24 API-key edit diff-before-patch), 2026-04-14 (Playwright E2E layer), 2026-04-13 (v0.1.25.23 nginx hotfix), 2026-04-13 (v0.1.25.22)
**Requires:** cycles-server v0.1.25.8+ (runtime plane, reservations dual-auth). Admin server v0.1.25.17+ continues to satisfy the governance plane; **admin server v0.1.25.18+ required** to execute the `RESET_SPENT` funding operation from BudgetsView; **admin server v0.1.25.26+ required** for the TenantsView / WebhooksView filter-apply bulk-action path (pre-.26 servers 404 the POST — the dashboard's row-select path continues to work unchanged); **admin server v0.1.25.29+ required** for the BudgetsView filter-apply bulk-action path shipped in v0.1.25.35 (pre-.29 servers 404 the `POST /v1/admin/budgets/bulk-action` — the dashboard's row-select fund flow continues to work unchanged); **admin server v0.1.25.29+ is now the shipped baseline** (pinned in both `docker-compose.prod.yml` and `docker-compose.yml` as of v0.1.25.35) — covers the BudgetsView bulk-action endpoint (v0.1.25.35), the AuditView error_code + status_band filters (shipped in v0.1.25.30 against admin .27), the AuditView filter DSL completeness (error_code_exclude + operation/resource_type arrays, shipped in v0.1.25.33 against admin .27), the OverviewView Recent-Denials pill drill-down, and the audit `tenant_id` sentinel split (`<unauthenticated>` → `__unauth__` + `__admin__`, admin .28); pre-.28 admins still work for the sentinel-split surfaces — dashboard's `TenantLink.isSystem` guard (v0.1.25.31) accepts both legacy and new sentinel conventions; pre-.27 admins ignore the unknown filter query params per the additive-parameter guarantee. **cycles-server v0.1.25.13+ is now the runtime-plane baseline** (pinned in both compose variants as of v0.1.25.32) for the `SORTED_HYDRATE_CAP = 2000` bound on `listReservationsSorted` — pre-.13 servers hydrate the full matching set before the in-memory sort and may OOM on tenants above ~2000 matching reservations; the client-side API surface is unchanged and pre-.13 servers continue to accept the same `sort_by`/`sort_dir` wire shape.

### 2026-04-18 — v0.1.25.37 (extension): row-select bulk failures → BulkActionResultDialog + EventTimeline correlation_id pivot

**Release scope.** In-place extension of v0.1.25.37 (PR #92 still open). Operator feedback after the Copy JSON work: *"is there anything else missing when it comes to triage, ops?"* Survey identified two gaps: (1) the four row-select bulk paths (TenantsView / WebhooksView / BudgetsView / TenantDetailView emergency freeze) still dropped per-row failures to `console.warn` even though the filter-apply paths in the same views routed through `BulkActionResultDialog`; (2) EventTimeline (BudgetDetail's recent-activity panel) rendered `correlation_id` as plain text, while EventsView already had a click-to-filter pivot — cross-view jump from a budget event to `/events?correlation_id=…` was missing. Both closed in this extension. No spec / server / compose dependency.

**What landed.**

1. **New utility** `src/utils/rowSelectBulkResult.ts` — `synthesizeRowSelectBulkResult({ targets, result, succeededIndices, idOf })` converts `rateLimitedBatch`'s `{ done, failed, cancelled, errors[] }` shape into a `{ succeeded, failed, skipped, total_matched }` response that plugs directly into the existing `BulkActionResultDialog`. The synthesizer:
   - Uses operator-supplied `succeededIndices` (captured by pushing from inside the worker on success — the only clean way to recover the settled-OK indices since `rateLimitedBatch` only tracks failures natively) to enumerate successes.
   - Extracts `error_code` from `ApiError.errorCode` and falls back to `toMessage(err)` for the per-row `message` field — matches the shape that `BulkActionResultDialog`'s `formatErrorCode` already consumes.
   - Marks cancelled-but-unreached targets as skipped with reason `"Not reached (operation cancelled)"` so the operator can retry just the tail instead of re-running the whole batch.
2. **TenantsView** (`src/views/TenantsView.vue:211-252`). `executeBulk` now captures `settledSucceeded: number[]` via an inline `.push(i)` inside the worker's success path; drops the `console.warn` loop; opens `BulkActionResultDialog` whenever `result.failed > 0 || result.cancelled`. Toast string `"check console for details"` → `"see details"`. `bulkResult` ref widened from `{ ...; response: TenantBulkActionResponse }` → `{ ...; response: TenantBulkActionResponse | RowSelectBulkResponse }` so the single `<BulkActionResultDialog v-if="bulkResult">` block in the template serves both the row-select and filter-apply paths.
3. **WebhooksView** (`src/views/WebhooksView.vue:198-247`). Same shape — `settledSucceeded` captured in-worker, dialog opened on failure/cancel, toast string updated, `bulkResult` response type widened.
4. **BudgetsView** (`src/views/BudgetsView.vue:594-665`). Same shape. Additionally builds `labelById` from `${b.scope} (${b.unit})` per target so each enumerated row in the dialog shows the scope-label as the primary line and the opaque `ledger_id` as the secondary mono line; passes `tenantId` only when the selected target set spans a single tenant (cross-tenant `over_limit` / `has_debt` selections leave `tenantId = ''` which suppresses the dialog's "View budget / View audit" triage links — those links require a tenant scope).
5. **TenantDetailView** (`src/views/TenantDetailView.vue:516-590`). `executeEmergencyFreeze` didn't have a result dialog at all pre-fix. Added `emergencyFreezeResult: Ref<{ actionVerb; response; labelById; tenantId } | null>` + new template block `<BulkActionResultDialog v-if="emergencyFreezeResult" item-noun-plural="budgets" :tenant-id="..." ... />`. Same `settledSucceeded` + `labelById` pattern as BudgetsView; `tenantId` always set (emergency freeze is always scoped to the current tenant detail), so the dialog's triage links work end-to-end for this path.
6. **EventTimeline correlation_id pivot** (`src/components/EventTimeline.vue:121`). `correlation_id` was rendered as plain `<span class="font-mono">`; rewritten to `<router-link :to="{ path: '/events', query: { correlation_id: ... } }">` so clicking the id jumps to EventsView filtered to the whole chain of events sharing that correlation. Cross-view pivot matches the within-view `viewCorrelated(id)` button that EventsView already exposes at `EventsView.vue:503`. `request_id` deliberately NOT wrapped — `listAuditLogs` and `listEvents` filter DSLs don't support `request_id`, so a deep-link would 404-equivalent (unfiltered result). Would need server-side filter additions in cycles-governance-admin and cycles-server before that pivot is wirable.

**Not in scope.** `request_id` cross-pivot from event → audit (requires server filter DSL addition); `correlation_id` filter on AuditView (same — `listAuditLogs` doesn't accept it); Copy icons next to individual IDs (redundant with the Copy JSON work already shipped in this version). Flagged in the PR description.

**Files.**
- NEW `src/utils/rowSelectBulkResult.ts` (73 lines).
- NEW `src/__tests__/rowSelectBulkResult.test.ts` (6 specs).
- `src/views/TenantsView.vue` — row-select executeBulk refit.
- `src/views/WebhooksView.vue` — row-select executeBulk refit.
- `src/views/BudgetsView.vue` — row-select executeBulkStatus refit + labelById / tenantId plumbing.
- `src/views/TenantDetailView.vue` — emergency-freeze dialog plumbing (new `emergencyFreezeResult` ref + template block).
- `src/components/EventTimeline.vue` — correlation_id → router-link.
- `src/__tests__/BudgetsView-row-select-bulk.test.ts` — 1 spec updated from `console.warn` assertion to `BulkActionResultDialog` assertion (the rest of the 10-spec file unchanged).

**Tests.** `src/__tests__/rowSelectBulkResult.test.ts` — 6 specs covering the synthesizer in isolation:
1. Enumerates all targets as succeeded when none failed.
2. Maps `errors[].index` into `failed[]` with id + error_code + message, lifting `errorCode` from `ApiError`.
3. Uses plain `toMessage` for non-ApiError failures and leaves `error_code` undefined.
4. Marks unreached targets as skipped when the batch was cancelled.
5. Partitions cleanly across all three arrays on the mixed-outcome cancelled path.
6. `total_matched` always reflects the input target count, not `done`.

`BudgetsView-row-select-bulk.test.ts:326` updated from "surfaces a failed-count summary when some per-row freezes fail" (asserted `console.warn` was called) to "opens per-row result dialog when some per-row freezes fail" (asserts `[role="dialog"][aria-label="Freeze budgets — results"]` exists and contains the failed scope).

**Validation gates (CLAUDE.md).**
- `npm run typecheck` clean.
- `npx vitest run` — **696 / 696 passing** across 55 files (+6 from the synthesizer spec).
- `npm run build` clean.

### 2026-04-18 — v0.1.25.37: Per-row "Copy JSON" on Events + Audit

**Release scope.** Small triage-UX addition. Operators asked for a way to copy a single event or audit entry into their clipboard as JSON — the existing bulk Export flows ship hundreds of rows when what's needed is often one row. No spec change, no server dependency. Compose admin lift `.29 → .30` paired in because admin-server `.30` shipped earlier the same day and the dev stack was already running against it.

**What landed on `feat/copy-row-as-json-v0.1.25.37`.**

1. **EventsView — widened Copy** (`src/views/EventsView.vue`). Old behavior: a `Copy` button inside the "Data" sub-box that copied only `event.data`. New behavior: a row-level `Copy JSON` button in the expanded-panel header (right-aligned above the 2-column metadata grid) that copies the **full Event object** via `safeJsonStringify(event)`. Fields preserved in the payload: `event_id`, `event_type`, `category`, `scope`, `tenant_id`, `source`, `timestamp`, `request_id`, `correlation_id`, `actor` (type + key_id), `data`. The narrower data-only copy is subsumed — the full event already contains `data`, and keeping both buttons would have been visual noise. Function `copyEventData(e)` → `copyEventJson(e)`; aria-label `"Copy data for event ${event_id}"` → `"Copy full JSON for event ${event_id}"`; button label `Copy` → `Copy JSON`. The `copiedEventId` ref + 2-second `setTimeout` debounce logic + silent clipboard-failure fallback all unchanged.
2. **AuditView — new Copy JSON button** (`src/views/AuditView.vue`). AuditView had zero copy affordance pre-.37 — operators were expected to select-and-copy from the metadata `<pre>` block or run a filtered export. New `Copy JSON` button in the expanded-panel header (right-aligned above the 2-column metadata grid, mirroring EventsView). Copies the **full AuditLogEntry including `metadata`** via `safeJsonStringify(entry)`. `metadata` is the critical field — spec v0.1.25.21 stows bulk-action per-row outcomes in `metadata.succeeded[] / .failed[] / .skipped[]`, and the audit endpoint's `search` param only scans `resource_id` / `log_id` / `operation` / `error_code` (not `metadata`), so metadata payloads aren't reachable from any UI filter. Copy JSON is the cleanest way to pull them into a ticket or pipe them through `jq`. Pattern cloned verbatim from EventsView: `copiedLogId: Ref<string | null>`, `copyLogJson(entry)`, 2-second `setTimeout` for label swap, silent fallback on clipboard rejection.
3. **EventTimeline — Copy JSON on expanded rows** (`src/components/EventTimeline.vue`). EventTimeline is the in-context event stream embedded in BudgetDetail (`BudgetsView.vue:1084`). Pre-.37 the expanded-row panel rendered the metadata grid + a `<pre>` block with the JSON-stringified data but no copy affordance; `JSON.stringify` inlined in the template (not `safeJsonStringify`). New `Copy JSON` button in the expanded-row panel, same position + styling as EventsView. Copies the **full Event object** via `safeJsonStringify(e)`. Same `copiedEventId` / 2-second-reset pattern. Data-box `<pre>` untouched — still shows just `e.data` for visual scanning, the row-level Copy JSON is the clipboard-export path.
4. **WebhookDetailView — inline Copy JSON on delivery rows** (`src/views/WebhookDetailView.vue`). Delivery rows are flat (no expand), so the pattern diverges: new trailing "Actions" column with an inline `Copy JSON` button on every row. Grid-template widened from 5 cols to 6: `'120px 120px 100px minmax(220px,1fr) 160px 88px'`. Column header reads "Actions". Copies the full `WebhookDelivery` (delivery_id, subscription_id, event_id, event_type, status, http_status, attempts, attempted_at, created_at, delivered_at) — useful for cross-referencing a failed delivery into `/events?event_id=…` or grep-matching against the receiver's server logs without hand-assembling the payload. `copiedDeliveryId` + 2-second reset pattern. No expand-panel work here since deliveries don't expand.

**Scope boundary.** Intentionally surgical. Budgets / Tenants / Webhooks / API Keys views already have dedicated detail views where the full object is visible and inline-selectable; no row-level Copy JSON needed there. The Events + Audit surfaces are the ones where rows collapse to a one-liner table cell with most fields hidden — that's the gap operators hit.

**Why no changes to bulk Export.** Bulk Export is ≥1 row and serializes through `useListExport` which is purpose-built for paginated downloads. A single-row copy via the same helper would be over-engineered — a direct `navigator.clipboard.writeText(safeJsonStringify(row))` is two lines and already matches the EventsView precedent.

**Files.**
- `src/views/EventsView.vue` — widened handler + relocated button (from Data sub-box → expanded-panel header); Copy-button removal from the Data sub-box row.
- `src/views/AuditView.vue` — new `copiedLogId` + `copyLogJson` + expanded-panel header button block.
- `src/__tests__/copy-row-as-json.test.ts` — NEW, 3 specs.
- `package.json`, `package-lock.json` — `0.1.25.36 → 0.1.25.37`.
- `docker-compose.prod.yml` — dashboard `.36 → .37`, admin `.29 → .30`.
- `docker-compose.yml` — admin `.29 → .30`.
- `README.md` — compose-example dashboard `.36 → .37`, admin `.28 → .30` (stale drift swept).

**Tests.** `src/__tests__/copy-row-as-json.test.ts` — 3 specs:
1. *"EventsView — copies the full Event object, not just e.data, and toggles label to Copied!"* — mounts EventsView with a single event, expands the row, clicks the Copy JSON button, asserts `navigator.clipboard.writeText` was called exactly once with a JSON payload containing `event_id`, `event_type`, `tenant_id`, `actor`, `data`; asserts label flip to `Copied!` then back to `Copy JSON` after `vi.advanceTimersByTime(2100)`.
2. *"AuditView — renders Copy JSON in the expanded panel and copies the full entry with metadata"* — same shape; asserts payload includes `log_id`, `operation`, `tenant_id`, `metadata.succeeded` (length 1), `metadata.failed[0].error_code` = `INVALID_TRANSITION`; asserts label toggle cycle.
3. *"AuditView — silently swallows clipboard errors"* — mocks `writeText` to reject once, asserts the click handler does not throw (permission-denied / insecure-context path).

**Validation gates (CLAUDE.md).**

- `npm run typecheck` (`vue-tsc -b --noEmit`) — clean.
- `npx vitest run` — **688 / 688 passing** across 54 files (+3 from v0.1.25.36's 685/53).
- `npm run build` — clean; EventsView chunk 15.25 kB gzip 5.22 kB, AuditView chunk 18.93 kB gzip 6.14 kB (both essentially unchanged — small handler + button block only).

**Backward compatibility.** Pure additive UI change on AuditView. EventsView's existing `Copy` button changed both semantics (data-only → whole-event) and label (`Copy` → `Copy JSON`); the label change is the signal to operators that scope widened. No deep-link, URL-param, or keyboard-nav regression.

**Spec alignment.** Unchanged at **v0.1.25.26**. No spec surface exercised. Compose admin `.30` is a convenience bump — admin-server `.30` was already running locally; no `.30`-specific dashboard features land in this release.
