...
### 2026-04-17 — Bulk-action wire-up: v0.1.25.21 filter-apply path (PR-B scaffold)

Wires TenantsView + WebhooksView to the `bulkActionTenants` / `bulkActionWebhooks` endpoints that cycles-governance-admin v0.1.25.21 specifies and **cycles-server-admin v0.1.25.26 will implement**. Scaffold ships as a draft PR held until the server lands; the additive type + client + UI path compiles and type-checks end-to-end today so the moment .26 goes live the draft can undraft with zero further dashboard work.
...
- Server-spec stream → `cycles-server-admin` v0.1.25.26 ships the actual bulk-action implementation. Until then, the draft PR sits unmerged; clicking the filter-apply button will 404.
...
**Deliberately held.** PR-B (bulk-action wire-up) stays unmerged until cycles-server-admin v0.1.25.26 ships the `bulkActionTenants` / `bulkActionWebhooks` endpoints. Proceeding with PR-A search-only keeps the dashboard shippable against admin v0.1.25.22 while .26 is in development — no feature-flag plumbing needed, just deferred merge.
