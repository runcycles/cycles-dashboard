...
| cycles-admin (governance plane) | v0.1.25.17+ | v0.1.25.55 | `.49`–`.51` align webhook matcher/ownership boundaries; `.52` hardens pagination, sorted-query limits, API-key collisions, tenant-close cascades, bulk idempotency, CORS traces, and request parsing; `.53` standardizes unsupported-method 405 responses; `.54` makes webhook-secret encryption fail closed unless plaintext is explicitly allowed for development; `.55` enforces non-negative policy priorities at the HTTP boundary. Public response compatibility is preserved; `.51+` should deploy after events `.23+`. |
...
Dependabot PRs #274–#284 refresh the shared TypeScript CI workflow reference,
checkout 7.0.1, OSSF Scorecard 2.4.4, Docker login 4.5.1, CodeQL SARIF upload
4.37.3, Playwright 1.62.0, ESLint 10.8.0, eslint-plugin-vue 10.10.0,
TypeScript ESLint 8.65.0, `@tanstack/vue-virtual` 3.13.34, and PostCSS 8.5.23.
Workflow actions remain SHA-pinned. The virtualization patch stays within the
existing API surface, while the remaining npm changes are development-only;
the PostCSS patch includes source-map loading hardening. There are no source,
API-request, UI-contract, spec-alignment, or server-fleet-pin changes. Each
reviewed or Dependabot-rebased head passed Node 20/22 tests with the 95%
coverage gate, strict typecheck, lint, production build, CodeQL, container
scanning, and applicable Compose/E2E checks.
...
The published server-fleet pin advances to admin 0.1.25.55 after its release
workflow and published-image smoke test passed; client-side validation remains
compatible with older supported servers. Validation:
...
focused protocol/presentation boundary. The parent retains acquisition,
table/copy/activity actions, tabs, routing, and capability coordination;
`TenantDetailView` drops from 1,276 to 1,150 lines.
...
| `useTenantDetailData` / view | Read generations and authoritative collection commits; tabs, capability gates, table actions, and sibling-owner coordination. |

**Confirmed fixes and hardening:**

- Create, edit, and revoke acquire publication ownership before their first
  write; older reads cannot overwrite authoritative edit/revoke responses or
  make a committed mutation appear retryable after settlement failure.
- Protocol guards enforce tenant/status eligibility, one-surface ownership,
  duplicate-submit and cancellation rules. Edit/revoke revalidate live rows;
  all executors recheck the external gate, covering disappeared/wrong-tenant
  keys and tenants closed after a dialog opened.
- API-key and tenant-lifecycle owners use reciprocal arming gates. The header,
  row menus, cascade recovery, Emergency Freeze, and lifecycle actions expose
  the protocol's disabled state; closed tenants also gate key creation/legacy
  active-row mutations.
- Create validates blank names and malformed date-time values before acquiring
  ownership; arrays are snapshotted, while legacy-permission healing and
  diff-only PATCH semantics remain intact. Revoke failures stay retryable and
...
Completes the fleet alignment started in `.66`: `cycles-server-admin`
`.47 → .48` now that its release exists. Admin `.48` is internal-only
(typed `EventDataTenantCascade` payload mapping; also silences the
payload-shape validator warnings on every tenant-close cascade) — no wire
change, no dashboard-visible behavior change, no operator action.

### 2026-07-04 — v0.1.25.66: fleet pin bump (server .46, events .22)

Bumps the bundled stack to the 2026-07-04 security releases: `cycles-server`
`.44 → .46` (default-on 429 rate limiting on the public evidence/JWKS
endpoints — anonymous surface only; dashboard traffic is far below the
300/min/IP limit) and `cycles-server-events` `.20 → .22` (delivery-time
SSRF guard). `cycles-server-admin` stays `.47` — `.48`
(EventDataTenantCascade payload mapping, internal-only) is merged but not
yet released; bump it on its next release. No dashboard code change.

Operational note carried into CHANGELOG: events `.22` re-validates webhook
URLs against the admin webhook-security config at delivery time, so stacks
delivering to `http://` or private-network receivers must set `allow_http`
/ adjust `blocked_cidr_ranges` via Settings → Webhook security, or those
deliveries permanently fail with `ssrf_blocked`. The dashboard's own
Webhook-security settings view is the remediation path, which is why the
note lives in this repo's operator docs too.

### 2026-07-03 — v0.1.25.65: spec-conformance audit (spec v0.1.25.35) + cascade event kinds in pickers

Full client-side audit against `cycles-governance-admin-v0.1.25.yaml`
v0.1.25.35 and the runtime-plane endpoints the dashboard drives. Result:
the API layer is conformant — all 40 endpoint paths/params match the spec
(admin plane + the AdminKeyAuth reservation/evidence surface), error parsing
follows `ErrorResponse`, pagination/sort/search/window params use the spec
names, and every wire enum field is typed as an open `string` with
unknown-value-safe rendering (raw text, `StatusBadge` gray fallback,
free-text filters) — the correct consumer posture per the spec's enum
EXTENSIBILITY rule.

One real gap, fixed: `EVENT_TYPES` lacked the four `*_via_tenant_cascade`
kinds (spec v0.1.25.35), which drive not just the Events type-filter
typeahead but the **webhook subscription event-type checkboxes** — cascade
events were un-subscribable from the UI (rendered/filterable, but not
pickable). Values added in their category groups; `eventTypes.test.ts` pins
the list to the spec enum (51 values, no dupes, every prefix a known
category). Spec badge → v0.1.25.35.

Also codified the strict ≥95% coverage rule in tooling: the vitest `lines`
threshold was a 70% floor while CLAUDE.md demanded 95 — raised to 95
(actual: 95.98%). Typecheck clean; 86 files / 984 tests green.

Flagged, not changed: no ESLint config exists and CI passes `run-lint:
false` — the `typecheck` script is the only static gate; statements/branches/
functions coverage (93.2/87.4/90.7) sit below the 95 line bar and keep their
70 floor.

### 2026-06-26 — v0.1.25.64: fleet pin alignment + container healthcheck
